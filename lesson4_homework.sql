...
DROP DATABASE IF EXISTS lesson4_homework;
CREATE DATABASE IF NOT EXISTS lesson4_homework;
...
SELECT shops.shopname, cat.name
FROM shops 
JOIN (SELECT * FROM cats
WHERE name = "Murzik") AS cat
ON shops.id = cat.shops_id;
...
JOIN cats
ON shops.id = cats.shops_id
WHERE name != "Zuza" AND name != "Murzik";
...
SELECT  analysis.an_name 'Анализ', analysis.an_price 'Цена', Orders.ord_datetime 'Время'
FROM analysis 
JOIN Orders
